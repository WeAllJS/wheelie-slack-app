  ...
  argv.respond(Team.get(argv.team_id).then(team => {
    ...
    return groups.listAsync({
      ...
      return chat.postMessageAsync({
        ...
        text: `Join request sent. Please wait while the request is processed.\n\nRemember that private channels are not for allies unless otherwise specified and that there is a strong expectation of privacy in these channels -- what is said in there stays in there. Note that the ${cocTxt} still fully applies in these spaces.`
      }))
    })
  }))
}
